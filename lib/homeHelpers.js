...
export const formatResponse = (text) => {
  ...
  // Format code blocks FIRST (to protect them from other formatting)
  formattedText = formattedText.replace(
    /```([^`]*(?:`{1,2}[^`]+)*[^`]*)```/g,
    '<pre style="background-color: #1e293b; padding: 16px; border-radius: 8px; margin: 16px 0; overflow-x: auto; white-space: pre-wrap; word-break: break-word; color: #e2e8f0; font-size: 14px; font-family: \'Courier New\', monospace; max-width: 100%; direction: ltr; text-align: left; border: 1px solid #334155;"><code>$1</code></pre>'
  );
  
  // Format inline code (protect from further processing)
  formattedText = formattedText.replace(
    /`([^`]+)`/g,
    '<code style="background-color: #f1f5f9; padding: 3px 8px; border-radius: 4px; color: #e11d48; font-family: \'Courier New\', monospace; font-size: 14px; border: 1px solid #e2e8f0; display: inline-block;">$1</code>'
  );
  
  // Format headers (with proper regex to avoid matching in code)
  formattedText = formattedText.replace(
    /^### (.+)$/gm,
    '<h3 style="font-size: 20px; font-weight: 600; color: #1f2937; margin: 20px 0 12px 0; line-height: 1.4;">$1</h3>'
  );
  formattedText = formattedText.replace(
    /^## (.+)$/gm,
    '<h2 style="font-size: 24px; font-weight: 700; color: #111827; margin: 24px 0 16px 0; line-height: 1.3; border-bottom: 2px solid #e5e7eb; padding-bottom: 8px;">$1</h2>'
  );
  formattedText = formattedText.replace(
    /^# (.+)$/gm,
    '<h1 style="font-size: 28px; font-weight: 800; color: #111827; margin: 28px 0 20px 0; line-height: 1.2;">$1</h1>'
  );
  ...
  // Format unordered lists (bullets) - simpler regex
  formattedText = formattedText.replace(
    /^[\-\*]\s+(.+)$/gm,
    '<li style="margin-left: 20px; margin-bottom: 8px; color: #374151; line-height: 1.6; list-style-position: inside;">• $1</li>'
  );
  
  // Format numbered lists
  formattedText = formattedText.replace(
    /^(\d+)\.\s+(.+)$/gm,
    '<li style="margin-left: 20px; margin-bottom: 8px; color: #374151; line-height: 1.6; list-style-position: inside;">$1. $2</li>'
  );
  
  // Convert line breaks to <br> for simple paragraphs
  formattedText = formattedText.replace(/\n/g, '<br>');
  
  // Wrap the entire content in a div with proper styling
  formattedText = `<div class="language-content markdown-content" style="color: #374151; font-size: 16px; line-height: 1.7; max-width: 100%; word-wrap: break-word; ${isRTL ? 'text-align: right; direction: rtl;' : 'text-align: left;'}">${formattedText}</div>`;
...
export const typeResponse = (content, setMessages, setIsTyping, shouldStop, setShouldStop) => {
  ...
  const typeInterval = setInterval(() => {
    ...
    if (index < characters.length) {
      // Process 2–3 characters per tick for natural ChatGPT-like typing speed
      const chunkSize = 3;
  ...
  }, 28); // ~100 chars/sec – natural typing speed like ChatGPT
};
    ...
    try {
      ...
      reader.onload = async function(event) {
        ...
        try {
          ...
          // Define typing animation function here so it's available in both try and catch blocks
          ...
          try {
            const response = await fetch('/api/chat', {
              ...
              body: JSON.stringify({
                message: `Analyze this PDF document and provide a comprehensive, structured summary:\n\n${fullText.substring(0, 15000)}`, // Limit to avoid token issues
                system: `You are an expert PDF document analyst. Create a thorough, well-structured analysis of the provided document.

**Your Analysis Must Include:**

1. **Document Type & Overview** (2-3 sentences)
   - Identify what type of document this is
   - State its main purpose and intended audience

2. **Key Information** (Organized sections with ## headers)
   - Main topics covered
   - Important facts, figures, and data points
   - Critical names, dates, or specific details
   - Core arguments or main points

3. **Detailed Content Analysis**
   - Break down into logical sections with clear headers
   - Extract specific details and insights
...
- Keep content factual and precise
- Organize information logically by importance

Provide a professional analysis that captures both the overall narrative and critical details. Be thorough and specific.`
              }),
            });
          ...
          // Provide a useful error message with helpful guidance
          const errorHtml = `<div style="font-family: system-ui, -apple-system, sans-serif; color: #111827; font-size: 16px; line-height: 1.6;">
            <div style="background-color: #fee2e2; border-left: 4px solid #ef4444; padding: 16px; margin-bottom: 16px; border-radius: 8px;">
              <h3 style="color: #b91c1c; margin: 0 0 8px 0; font-weight: 600; font-size: 18px;">❌ PDF Processing Error</h3>
              <p style="color: #ef4444; margin: 0; font-size: 15px;">Error: ${error.message || "Unknown error occurred"}</p>
            </div>
            
            <h4 style="color: #1f2937; font-weight: 600; margin-bottom: 12px;">🔍 Possible Causes:</h4>
            <ul style="padding-left: 20px; margin-top: 5px; color: #374151; list-style-type: none;">
              <li style="margin-bottom: 12px; padding-left: 8px; border-left: 3px solid #3b82f6;">
                <strong>🔒 Protected PDF:</strong> The file is encrypted or password-protected
              </li>
              <li style="margin-bottom: 12px; padding-left: 8px; border-left: 3px solid #3b82f6;">
                <strong>📷 Scanned Document:</strong> Contains images instead of searchable text (needs OCR)
              </li>
              <li style="margin-bottom: 12px; padding-left: 8px; border-left: 3px solid #3b82f6;">
                <strong>💔 Corrupted File:</strong> The PDF structure is damaged or malformed
              </li>
              <li style="margin-bottom: 12px; padding-left: 8px; border-left: 3px solid #3b82f6;">
                <strong>📦 Large File:</strong> File is too large (current: ${Math.round(file.size / 1024)} KB, recommended: <5 MB)
              </li>
              <li style="margin-bottom: 12px; padding-left: 8px; border-left: 3px solid #3b82f6;">
                <strong>🎨 Complex Layout:</strong> Contains complex graphics or unusual formatting
              </li>
            </ul>
            
            <div style="background-color: #f0f9ff; border-left: 4px solid #3b82f6; padding: 16px; margin-top: 20px; border-radius: 8px;">
              <h4 style="color: #1e40af; margin: 0 0 12px 0; font-weight: 600;">💡 Suggestions to Fix:</h4>
              <ol style="padding-left: 20px; margin: 0; color: #1e3a8a;">
                <li style="margin-bottom: 8px;">Remove password protection if the PDF is encrypted</li>
                <li style="margin-bottom: 8px;">Try uploading a different, smaller PDF file</li>
                <li style="margin-bottom: 8px;">Use a text-based PDF (not scanned images)</li>
                <li style="margin-bottom: 8px;">Compress the PDF if it's larger than 5MB</li>
                <li style="margin-bottom: 8px;">Export the PDF again from the original source</li>
              </ol>
            </div>
            
            <p style="margin-top: 20px; text-align: center; color: #6b7280; font-style: italic;">
              Need help? Contact support or try uploading a different document.
            </p>
          </div>`;
